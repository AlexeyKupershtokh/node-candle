...
// Then it sends a distributed request according to the shard map.
// Each server responds with a delay.
// It passes a payload containing its origin server number.
// Since the server1 sets a timeout of 100ms, then no replies from the server4
// (where delay is 120ms) will be included into results.
...
// collector is an utility for collecting responses. Since it has collected the
// target number of collectibles, it calls the handler function passing the collection to it.
...
function distributed_request(requests, handle) {
  // create a collector
  var i, id, socket, collect = collector(requests.length, handle);
  for (i = 0; i < requests.length; i++) {
    ...
    socket = sockets[shard_map[requests[i]]];
    // send a request
